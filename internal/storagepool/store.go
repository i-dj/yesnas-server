...
func Add(pool StoragePool, devices []PoolDevice) error {
	...
	if _, err := tx.Exec(
		`INSERT INTO storage_pool (id, storage_id, name, filesystem, raid_level, mount_path, data_path, cache_mode, read_speed_bytes_per_sec, write_speed_bytes_per_sec, speed_tested_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		pool.ID, pool.StorageID, pool.Name, pool.Filesystem, pool.RaidLevel, pool.MountPath, pool.DataPath, pool.CacheMode, pool.ReadSpeedBytesPerSec, pool.WriteSpeedBytesPerSec, pool.SpeedTestedAt, pool.UpdatedAt,
	); err != nil {
		return err
	}
